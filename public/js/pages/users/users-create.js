/**
 * Users Create Page JavaScript
 * Handles user creation form using existing form helpers
 */
...
App.pages.usersCreate = {
    ...
    init: function() {
        ...
        // Only initialize if we're on the user create page
        if (!this._form) {
            return;
        }
    ...
    _setupForm: function() {
        ...
        if (App.utils.storage) {
            App.utils.storage.remove(this._autoSaveKey);
        }
    },
    ...
    _handleSuccess: function(data, form) {
        ...
        this._showNotification('User created successfully!', 'success');
    ...
    _handleError: function(error, form) {
        if (error.status === 422) {
            ...
            this._showNotification('An error occurred while creating the user.', 'error');
        }
    },
